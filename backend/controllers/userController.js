...
exports.updateProfile = async (req, res) => {
  try {
    ...
    await db.query(
      `UPDATE users SET name = ?, email = ?, phone = ?, latitude = ?, longitude = ? WHERE id = ?`,
...
exports.updateWallet = async (req, res) => {
  try {
    ...
    if (existing.length > 0) {
      await db.query(
        `UPDATE wallets SET cash_amount = ?, upi_amount = ? WHERE user_id = ?`,
        [cash, upi, userId]
      );
    } else {
      await db.query(
        `INSERT INTO wallets (user_id, cash_amount, upi_amount) VALUES (?, ?, ?)`,
        [userId, cash, upi]
      );
    }

    res.json({ message: 'Wallet updated successfully', wallet: { cash, upi } });
  } catch (error) {
...
exports.updateLocation = async (req, res) => {
  try {
    ...
    res.json({ message: 'Location updated successfully', location: { lat: latitude, lng: longitude } });
  } catch (error) {
